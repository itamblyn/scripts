...
# by default. If argument is supplied that file will be opened.
  ...
  if(@ARGV == undef){
    if(-e "XDATCAR"){
      ...
      $xdatzipped = 1 ; }
    else{ die " NO XDATCAR IN THIS DIRECTORY \n" ; }
    print " OPEN XDATCAR ... \n" ;
    open XDAT, "XDATCAR"
...
#      while($z<-0.5){$z+=1.0;} while($z>0.5){$z-=1.0;}
  ...
  close XDAT;
  if($xdatzipped){print " ZIPPING XDATCAR AGAIN \n" ; system "$zip -9 XDATCAR &" ;}
