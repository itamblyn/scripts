...
#;-*- Perl -*-
#####################################################################################
# It makes movies for vibrational modes calculated through dymmat                   #
# written by Lijun Xu and Graeme Henkelman in the Univeristy of Texas at Austin     #
# Last modified by Lijun on May 9, 2007                                             #
#####################################################################################
use FindBin qw($Bin) ;
use lib "$Bin" ;
use Vasp ;

@ARGV>2 || die "Usage: dymmodes2xyz.pl POSCAR DISPLACECAR modes.dat <moviefolder> <freq.dat> <numimages> <dist>\n";
$poscar=$ARGV[0];
$displacecar=$ARGV[1];
$modesfile=$ARGV[2];
$moviefolder=".";
