...
# by Lijun Xu, Wenjie Tang and Graeme Henkelman (The University of Texas at Austin) #
# (last modified by Graeme on May. 7, 2007, UT Austin)                              #
# version 1.0: two-image dimer method                                               #
# version 2.0: single-image dimer method replaces the  two-image method             #
# version 3.0 (proposed): add LANZCOX (single image) to the code                    #
#####################################################################################
use FindBin qw($Bin);
use lib "$Bin";
use Vasp;
...
$SimR = ReadParm($configfile,"SimR", "0.1");
...
($SimR>0) || die "Error: no SimR, the distance that the dimer images are separated to initialize minimizations\n";
if($screenoutput) {
...
$index=ReadIndex($AkmcFile);
$step=0;
sub numerically {$a <=> $b}
@lasttwosteps=(sort numerically keys %$index);
...
for $j (@lasttwosteps){ # there should be only one member i.e., the last step
  $step=$j;
  #print "step $j: @{$index->{$j}}\n";
#  print "$j. Info: state ".${$index->{$j}}[0].", ".${$index->{$j}}[1].", ".${$index->{$j}}[2].", energy ";
  print "$j, state ".${$index->{$j}}[0].", ".${$index->{$j}}[1].", energy ";
  printf ("%3.3f",${$index->{$j}}[3]);
  print " eV.\n";
}

if($step == 0) { #no $AkmcFile, i.e., minimize the initial configuration
  print "Initialize the first configuration ...\n";
  ($ready,$numrunjobs,$jobs,$index)=Initializer($min,$numrunjobs,$MaxJobs,$jobs,$index);
  if($ready) {
    ...
    #index hash should have a key "1" and a list value starting from "2bcreated"
    $curstep="1";
    $Done=0;
  }else{
    $curstep="0";
  ...
  if($curst eq $index->{$step}[0]) {
    ...
    if($normal) {
      ...
      $prest=$index->{$step}[2];
      $curstep=$step+1;
      $index->{$curstep}=[("2bcreated","na","na",$index->{$step}[4],"na",$index->{$step}[5])];
      print "We are trying to continue from a finished or stopped akmc run.\n";
    }
  }else{ # Create the index for the current step
    $cur_eq_pre=0;
    if($normal) { # We are working on an unfinished state
      $prest=$index->{$step}[2];
      $curstep=$step+1;
      $index->{$curstep}=[($curst,"new",$curst,$index->{$step}[4],"na",$index->{$step}[5])];
...
while(!$Done){
  ...
  $curdir=$index->{$curstep}[0];
  $repeat=0;
  if($curdir eq "2bcreated"){ # working on the home poscar
    # check the current state pool to see if this is a new state.
    ($repeat,$index)=CheckStpool($curstep,$stpool,$StEnergyFile,$equivalency,$Ediffmax,$Rdiffmax,$index);
    if(!$repeat) {
      print "Current (new) state is: ";
      $NumSt++;
      $curdir=DirName("st",$NumSt);
      ($curdir eq $index->{$curstep}[0]) || die "error: index{currentstep}[0] is not same as curdir\n";
      MakeNewSt($curdir);
      $index->{$curstep}[2]=$curdir;
      print "$curdir\n";
    }else{
      $curdir=$index->{$curstep}[2];
      print " current st(repeated): same as $index->{$curstep}[0] in $index->{$curstep}[2]".".\n";
    }
  }elsif($index->{$curstep}[1] eq "new" && $curdir eq DirName("st",$NumSt)){ # working on the latest state
    if(-e $StEnergyFile){
      chomp($lxtwj=`grep $curdir $StEnergyFile | tail -1`);
      if($lxtwj ne "") {$index->{$curstep}[1]="repeat";}
    }
    if($index->{$curstep}[1] eq "repeat") {
      $repeat=1;
      print "Current state (repeated) is: same as $index->{$curstep}[0] in $index->{$curstep}[2]".".\n";
    }else{
  ...
  if($repeat) {
    if($ststatus eq "done") {
      ...
      die "We are dealing with a repeated st ($curdir) which must have been marked as done.\n";
    }
  }
  #---------------------------------------------------------------------------
  # Update current state information
  #---------------------------------------------------------------------------
  $curstenergy=$index->{$curstep}[3];
  if(exists($st->{$curdir})) {
    if(abs($curstenergy - $st->{$curdir}[0]) > 0.01 ) { # 0.01 ev is the threshold, normally they should be almost exactly same.
      die "The states are messed up. Check the next state energy in the previous state index\nand the config at home which should be the current state configuration.\n";
    }
  }
  if($ststatus eq "NOST"){
    print "State $curdir is new with no processes.  Creating a new state and quiting.\n";
    # prepare sp and dynmat jobs and submit them.
    ($jobs,$st)=StartSt($curdir,$MaxJobs,$NumSearches,$SearchesAlgo,$quench,$dynmat,$turnoff_dynmat,$curstep,$stpool,$PrRecycleAll,$PrRecycle,$PrRecycleShift,$SurfaceRecShift,$StEnergyFile,$StFile,$equivalency,$Ediffmax,$Rdiffmax,$DisplaceAlgo,$DisplaceRange,$NN_rcut,$MaxCoordNum,$index,$jobs,$st); 
    $numprrun=$st->{"numprst"}[0];
    $numprfin=0;
    $numrunjobs=$MaxJobs;
    if($curstep == 1) {WriteIndex($AkmcFile, $index, 1);}
    $Done=1;
    ...
    for($i=1; $i<=$numprst; $i++){
      if($st->{"NumSearchesLeft"}[0]==0) {# state is running with no searches
        ...
        print "Waiting for the dynamical matrix or the state configuration to finish.\n"; 
        last;
      }
      $prdir=DirName("pr",$i);
      ...
      if($prstatus eq "done"){
        ...
        $spenergy=$st->{$prdir}[2];
        ($numrunjobs,$quenchst,$jobs,$comment)=HandleQuench($curspquendir,$SearchesAlgo,$numrunjobs,$MaxJobs,$SimR,$Ediffmax,$Rdiffmax,$spenergy,$jobs);
        #print "... now it is ";
        if($quenchst eq "completed"){
          #print "completed.\n";
        ...
        }elsif($quenchst eq "pushmore"){
          #print "needs be re-started by pushing two images further more\n";
          delete($jobs->{$curspquendir."/min1"});
          delete($jobs->{$curspquendir."/min2"});
          ($numrunjobs,$jobs,$quenstatus,$comment)=StartQuench($curspdir,$SearchesAlgo,$numrunjobs,$MaxJobs,2*$SimR,$jobs);
          $UPDATEPROCESS{"jobstatus"}="run";
          if($quenstatus eq "killed"){
            $st->{$prdir}[0]="killed";
          ...
          $numprrun++;          
        }elsif($quenchst eq "running"){
          #print "still running(or to be submitted).\n";
          $numprrun++;
          $UPDATEPROCESS{"jobstatus"}="run";
        }elsif($quenchst eq "killed"){
          $st->{$prdir}[0]="killed";
          $st->{$prdir}[1]=$comment."_quench";
          $numprkilled++;
          #print "killed.\n";
        ...
        #print "***********************\n";
        #print "updating ...\n";
        ($numrunjobs,$jobs,$comment)=HandleSP($curdir,$prdir,$SearchesAlgo,$SPEnergyMax,$numrunjobs,$MaxJobs,$jobs);
        #print " ... now it is\n";
        if(lc($jobs->{$curspdir}[0]) eq "completed"){
          #print "completed.\n";
          $st->{$prdir}[2]=$jobs->{$curspdir}[3]; # get the saddle point energy;
          $st=FilterSP($curdir,$prdir,$numprst,$BarrierMax,$SearchesAlgo,$quench,$dynmat,$StFile,$turnoff_dynmat,$equivalency,$stpool,$GrabQuenched,$Ediffmax,$Rdiffmax,$DynMatFile,$stdirs,$st);
          @repeat_prdir=split(/\*/, $st->{$prdir}[1]);
          if(lc($repeat_prdir[0]) eq "repeat"){
            ...
            ($numrunjobs,$jobs,$quenstatus,$comment)=StartQuench($curspdir,$SearchesAlgo,$numrunjobs,$MaxJobs,$SimR,$jobs);
            $st->{$prdir}[0]="quench";
            $UPDATEPROCESS{"jobstatus"}="run";
            if($quenstatus eq "killed"){
              $st->{$prdir}[0]="killed";
          ...
          }elsif(lc($st->{$prdir}[1]) eq "trivial"){
            #print "but it turns out to be a trivial saddle. The process is halted.\n";
            $st->{$prdir}[0]="done";
            $numprfin++;
            $comment="TrivialSaddle";
      ...
      if($ConvergenceAlgo == 1 || $ConvergenceAlgo == 0){ # check the state status at every step
        if(!$alreadydone && (lc($st->{$prdir}[0]) ne lc($st_old->{$prdir}[0]))) { # process status is different
    ...
    # check if all the process in the current st have been finished and evaluate the population of found good saddle point
    ...
    ($numprfin,$numprrun,$numrunjobs,$jobs,$st)=CheckSt($curdir,$numprfin,$numprrun,$numrunjobs,$MaxJobs,$NumSearches,$SearchesAlgo,$ConvergenceAlgo,$Population,$NumKT,$Temperature,$DisplaceAlgo,$DisplaceRange,$NN_rcut,$MaxCoordNum,$jobs,$st,$st_old);
  ...
  if(lc($st->{"status"}[0]) eq "done"){
    ...
    $rdnum=rand();
    $rdnum2=rand();
    ($selected,$elapse,$enforced,$time_saved)=SelectEvent($rt,$curdir,$prest,$rdnum,$rdnum2,$enforced,$time_saved,$Rdiffmax,$RateTableFile,$Temperature,$RateTableFileBolEqu,$index,$curstep,$BoltzmanEqu);
#-------------------------------------------------------------------------------------------------------------------------
    print "The chosen process for $curdir is $selected.\n";
    if($enforced eq ""){print "Process $enforced is enforced.\n";}
    # save the state and update the index after making a move:
    # setup index for next but state needs to be taken care of later.
    ($curstep,$st,$index)=UpdateSt($curstep,$repeat,$selected,$rdnum,$elapse,$StFile,$StEnergyFile,$AkmcFile,$st,$index);
...
sub ReadParm{
  ...
  while($line=<CONFIG>){
    $line=~s/^\s+//;
    @line=split(/\s+/,$line);
    if(lc($line[0]) eq lc($parm) and $line[1] eq '=') {
      $value=$line[2]; 
      #print $parm."=".$value."\n";
    }
  }
...
sub CheckSystemConfig{
  ...
  my $curst="";
  my @dist=();
  my ($homeposcar, $curst_poscar, $dist);
...
# -----------------------------------------------
# Read index [%index=ReadIndex($AkmcFile)]
# -----------------------------------------------
sub ReadIndex{ # read the last two steps information
  my $AkmcFile=shift;
  my $line="";
  my @line=();
  my %index=();
  ...
  if(-e $AkmcFile){
    ...
    for($step=0; $step< 1;$step++) {
      ...
      if($steps[0]=~/^\d+$/) {
        ...
        $index{$steps[0]}=[()];
        for ($j=1;$j<@steps;$j++) {
          push @{$index{$steps[0]}}, lc($steps[$j]);
        }
      }
    }
    if(!$linesize) {die "Error In ReadIndex, the last line of $AkmcFile is blank. Go fix it.\n";}
  }
  return \%index;
}
...
# ---------------------------------------------------
# Initialize aKMC: Initializer($min,$numrunjobs,$MaxJobs,$jobs,$index)
# ---------------------------------------------------
sub Initializer{
  my ($min,$numrunjobs,$MaxJobs,$jobs,$index)=@_;
  my $curdir=$min;
  my $ready=0;
  my $energy="na";
  my $force="na";
  ...
  $dummy="1";
  $index->{$dummy}=[()];
  push @{$index->{$dummy}}, "2bcreated","unkonwn","unknown",$energy,"na","0.0";
  return ($ready, $numrunjobs,$jobs, $index);
}
...
# ------------------------------------------------------------------------------------------------------------
sub CheckStpool{
  my ($curstep,$stpool,$stenergyfile,$equivalency,$Ediffmax,$Rdiffmax,$index)=@_;
  my $repeat=0;
  my $i=0;
  my $j="";
  ...
  for $j (@totalst){
    if (abs($energy-$stenergy->{$j}) < $Ediffmax) {
      ...
      if($same) {
        $repeat=1;
        $index->{$curstep}[0]=$j;
        $index->{$curstep}[1]="repeat";
        $index->{$curstep}[2]=$j;
        last;
      }
    }
  }
  if($repeat){ return ($repeat, $index);}
  ...
  if(!$repeat){
    $index->{$curstep}[0]=DirName("st", $numst+1);
    $index->{$curstep}[1]="new";
    $index->{$curstep}[2]="na";
  }
  return ($repeat, $index);
...
sub SubmitJobs{
  ...
  if(!$filemissing && $POSCAR_size > 6){
    ...
    chomp($jobinfo=`cd $curdir; $Bin/akmc_submit.pl $presentworkdir | tail -1;`);
    $jobinfo=~s/^\s+//;
    @jobinfo=split(/\s+/,$jobinfo);
    $jobid=$jobinfo[0];
    if($jobid=~/^\d+$/) { # a digital id
      $jobstatus="submitted";
    }elsif(lc($jobid) eq "2bsubmitted") {
    ...
    }else{ #at least of one vasp basic files is missing: INCAR, POTCAR, KPOINTS, POSCAR and akmc.sub
      $jobstatus="killed";
      $jobid="MissingBasicVaspFiles";
    }
  }elsif($filemissing){
...
sub KillJobs{
  ...
  my $line="";
  my @line=();
  chomp($line=`cd $curspdir; $Bin/akmc_kill.pl $jobid | tail -1;`);
  $line=~s/^\s+//;
  ...
  if((-e "$curspquendir/min1")&&(-e "$curspquendir/min2")){# there are quench mins
     for($j=1;$j<3;$j++){
       ...
       if(lc($st->{$prdir}[0]) eq "quench"){ #quenching
         ...
         }elsif(lc($st->{$prdir}[1]) eq "bad" || lc($st->{$prdir}[1]) eq "trivial"){
           $line.="";
         }
       }elsif(lc($st->{$prdir}[0]) eq "killed"){
...
sub PatchupRecycledJobInfo{
  ...
  $jobs->{$curspdir}=[("completed","12345",$SearchesAlgo,$energy,$force,$curvature)];
  ...
  for($j=1;$j<3;$j++){
    ...
    $jobs->{$minj}=[("completed","1234".$j,"minimization",$energy,$force)];
  }
  #get dynmat information
  if(-e "$curspdyndir/final"){# it is assumed done here
    $jobs->{$curspdyndir}=[("completed","12343","dynmat","na","na")];
  }
  return $jobs;
}
# -----------------------------------------------------------
  ...
  if((-e "$prdir/final/OUTCAR.gz") || (-e "$prdir/final/OUTCAR.bz2") || (-e "$prdir/final/OUTCAR")){
    ...
    if($check){
      ...
      while($line=<incar>){
        ...
        $dummy[0]=~s/^\s+//;
        chomp($dummy[0]);
        $dummy[0]+=0; # get rid of possible trailing whitespace
        if(lc($line[0]) eq "ediffg" && $dummy[0]=~/^[-+]?\d+(\.\d+)?([eE][-+]?\d+)?$/ && $dummy[0]< 0)  {
          $fmax=abs($dummy[0]);
        }elsif(lc($line[0]) eq "ediff" && $dummy[0]=~/^[-+]?\d+(\.\d+)?([eE][-+]?\d+)?$/){
          $ediff=$dummy[0];
        }
      }
      close incar;
...
sub CheckDynmat{
  ...
  my $zip = $ENV{'VTST_ZIP'} ;
  if($zip eq ''){ $zip = 'gzip' ; }
  if((-e "$curpredir/OUTCAR") || (-e "$curpredir/OUTCAR.gz") || (-e "$curpredir/OUTCAR.bz2")) {
...
sub CleanupJobs{
  ...
  if($good){
    ...
    $yes=FinishOneIonicCycle($curdir,$jobtype);
    if(!$yes) {system "cd $curdir; cp POSCAR CONTCAR";}
  }
  # make sure ll_out etc. is removed
  system("cd $curdir; $Bin/vfin.pl $foldername > /dev/null");
  if(!$yes){system "cd $curdir/$foldername;rm -f CONTCAR CONTCAR.con CONTCAR.xyz";}
  return;
}
...
# ---------------------------------------------------------------------------------------
# Calculate normal mode frequencies [($twonegatives,$dynmatfilepath)=DynMatrix($curpredir,$DynMatFile,$max)]
# ---------------------------------------------------------------------------------------
sub DynMatrix{
  my ($curpredir,$DynMatFile,$maximumnegative)=@_;
  my ($dynmatfilepath,$curpredirdyn);
  my $imaginery=0;
  my $twonegatives=0;
  my @line=();
  my $outzipped=0;
  ...
  $dynmatfilepath=$curpredirdyn."/".$DynMatFile;
  if(-e "$dynmatfilepath") {system "rm $dynmatfilepath";}
  if(-e "$curpredirdyn/OUTCAR.gz") {system "$outzipped=1; cd $curpredirdyn; gunzip OUTCAR.gz";}
  elsif(-e "$curpredirdyn/OUTCAR.bz2") {system "$outzipped=1; cd $curpredirdyn; bunzip2 OUTCAR.bz2";}
  $dummy=`cd $curpredirdyn; $Bin/dymmatrix.pl DISPLACECAR OUTCAR`;
  #print "dynmat dir: $curpredir"."\n";
  if(-e "$dynmatfilepath"){
    open(DYNMAT, "<$dynmatfilepath") || die "in $curpredirdyn: $dynmatfile cannot be opened.\n";
    while($line=<DYNMAT>) {
      $line=~s/^\s+//;
      @line=split(/\s+/,$line);
    ...
    die "in $curpredirdyn: $dynmatfile is not successfully created by dynmatrix.pl\n";
  }
  if($outzipped) {system "cd $curpredir; $zip OUTCAR";}
  return ($twonegatives,$dynmatfilepath);
}
...
sub CompareTwoPOSCAR{
  ...
  my ($basis,$lattice,$num_atoms,$selectiveflag,$selective,$description);
  my ($same, $i, $j, $differ,$poscarfilename);
  #print " Reading POSCAR1:$curspdir\n";
  $poscarfilename=$curspdir;
  ($coordinates1,$basis,$lattice,$num_atoms,$total_atoms1,$selectiveflag,$selective,$description)=read_poscar($poscarfilename);
  set_bc($coordinates1,$total_atoms1);
  #print " Reading POSCAR2:$curpsdir\n";
  $poscarfilename=$curpsdir;
  ($coordinates2,$basis,$lattice,$num_atoms,$total_atoms2,$selectiveflag,$selective,$description)=read_poscar($poscarfilename);
  ...
  $differ=pbc_difference($coordinates1,$coordinates2,$total_atoms1);
  $differ=dirkar($differ,$basis,$lattice,$total_atoms1);
  $same=1;
  for($i=0;$i<$total_atoms1;$i++){
    for($j=0;$j<3;$j++){
      if(abs($differ->[$i][$j]) > $Rdiffmax) {
        $same=0;
        return $same;
      }
    }
...
# --------------------------------------------------------------------
# Check if two POSCARs are indistinguishable: return $same
# --------------------------------------------------------------------
sub indistinguishable{
  my ($poscar1,$poscar2,$Rdiffmax)=@_;
  my ($coordinates1,$coordinates2,$total_atoms1,$total_atoms2);
  my ($basis,$lattice,$num_atoms1,$num_atoms2,$selectiveflag,$selective,$description);
  my ($same,$i,$j,$k,$m,$differ,$matched,$mark,$start,$end,%included);
  my %linker=();
  my @dummy=();
  #print " Reading POSCAR1:$poscar1\n";
  ($coordinates1,$basis,$lattice,$num_atoms1,$total_atoms1,$selectiveflag,$selective,$description)=read_poscar($poscar1);
  set_bc($coordinates1,$total_atoms1);
  #print " Reading POSCAR2:$poscar2\n";
  ($coordinates2,$basis,$lattice,$num_atoms2,$total_atoms2,$selectiveflag,$selective,$description)=read_poscar($poscar2);
  set_bc($coordinates2,$total_atoms2);
...
sub ReshufflePOSCAR{
  ...
  my ($basis,$lattice,$num_atoms,$selectiveflag,$selective,$description,$total_atoms);
  my ($i,$j,$k,$coordinates,$R,$Select);
  ($coordinates,$basis,$lattice,$num_atoms,$total_atoms,$selectiveflag,$selective,$description)=read_poscar($poscar2);
  ...
  write_poscar($R,$basis,$lattice,$num_atoms,$total_atoms,$selectiveflag,$Select,$description,$poscar1);
}               
...
sub ReshuffleMins{
  my ($mins,$linker)=@_;
  my $min1=$mins."/min1/POSCAR";
  my $min2=$mins."/min2/POSCAR";
  my @targets=($min1, $min2);
  ...
  open(README, ">$readme");
  print README "Attention: only the following files were reshuffled. They may be different in atom sequence compared to files in final/ etc.\n";
  print README "@targets\n";
  close README;
}
...
sub ReshuffleProcess{
  ...
  my $POSCAR_sp=$prdir."/POSCAR_sp";
  my $readme=$prdir."/readme";
  ReshufflePOSCAR($POSCAR_sp,$POSCAR_sp,$linker);
  ReshuffleMins($mins,$linker);
  open(README, ">$readme");
  print README "Attention: $POSCAR_sp was reshuffled. It may be different in atom sequence compared to files in final/ etc.\n";
  close README;
} 

# --------------------------------------------------------------------------------------
# Calculate the radiator for two poscar files
#($rad)=Radiator($start,$end,$coordinates,$total_atoms);
# --------------------------------------------------------------------------------------
sub Radiator{
...
# --------------------------------------------------------------------------------------
# $rad_dis_f=CalcRadialDistFunc($coordinates,$total_atoms,$basis,$lattice,$rad_dis_f1,$j,$size_bins,$amin)
# --------------------------------------------------------------------------------------
...
sub EvaluateTwoPOSCAR{
  ...
  my ($coordinates1,$coordinates2,$total_atoms1,$total_atoms2,$num_atoms1,$num_atoms2,$selective1,$selective2);
  ...
  my %linker=();
  my @dummy=();
  my ($thatisit,$similarity,$samei,$checkedatoms);
  $thatisit="";
  $similarity=0;
  $numfrozen=0;
  $checkedatoms=0;
  ...
  ($coordinates1,$basis,$lattice,$num_atoms1,$total_atoms1,$selectiveflag,$selective1,$description)=read_poscar($poscarfilename);
  set_bc($coordinates1,$total_atoms1);
  #print " Reading POSCAR2:$curpsdir\n";
  $poscarfilename=$POSCAR_j;
  ($coordinates2,$basis,$lattice,$num_atoms2,$total_atoms2,$selectiveflag,$selective2,$description)=read_poscar($poscarfilename);
  set_bc($coordinates2,$total_atoms2);
  if($total_atoms1 == $total_atoms2 && @$num_atoms1 == @$num_atoms2) {
    #print "@$num_atoms1\n";
    $mark=@$num_atoms1 - 1;
      ...
      if($num_atoms1->[$i] == $num_atoms2->[$i]) {
        ...
        for($j=$start;$j<$end;$j++) { # locating a group of atoms
          ...
          for($k=$start;$k<$end;$k++) {
            ...
            if($matched) {
              if(!(exists($included{$k}))) {
                ($same,$rad_dis_f1,$rad_dis_f2)=CheckRadialDistFunc($coordinates1,$coordinates2,$total_atoms1,$basis,$lattice,$rad_dis_f1,$j,$rad_dis_f2,$k,$dummy[0]);
                #print "@{$rad_dis_f1->{$j}}\n";
                #print "@{$rad_dis_f2->{$k}}\n";
...
sub exp_repulsion_pot{ # E=exp(kx)-b,k<0 #not good to use.
  my ($r,$cutoff)=@_;
  my $slope=-0.3;
  my ($energy, $fij);
...
sub GrabQuenchedSP{
  ...
  for($i=0;$i<$stpoolsize;$i++) {
    $headpath=$stpool->[$i];
    opendir MAINSTDIR, "." or die "can't open this dir!";
    ...
    if((lc($stinfo->{$pr000i}[1]) eq "good") && (lc($stinfo->{$pr000i}[0]) eq "done" || lc($stinfo->{$pr000i}[0]) eq "dynmat")) {
      if(abs($energy-$stinfo->{$pr000i}[4]) < $Ediffmax) {
        ...
        if($same) {
          ...
          system "cd $stdir; cp -r ../$filepath $newprdir; cd $newprdir; cd $quench; mv min1 mininter; mv min2 min1; mv mininter min2";
      ...
      if(abs($energy-$stinfo->{$pr000i}[4]) < $Ediffmax) {
        ...
        if($same) {
          ...
          $filepath=$target."/".$pr000i;
          $newprdir=DirName("pr",$recovered);
          system "cd $stdir; cp -r ../$filepath $newprdir";
          if($equivalency && $yes) {ReshuffleProcess($stdir."/".$newprdir,$quench,$linker);}
          $st->{$newprdir}=$stinfo->{$pr000i};
          $st->{$newprdir}[1]="good";
...
# $stpool,$stenergyfile,$stfile,$recovered,$included,$equivalency,$Ediffmax,$Rdiffmax,$stprs,$index,$jobs,$st)
# ---------------------------------------------------------------------------------------------------------
sub UseGoodBad{
  my $stdir=shift;
  my $curstep=shift;
  ...
  my ($newprdir,$pr000i,$target,$sp1, $sp2,$repeated,$repeat,$previousst,$totalsteps);
  $energy=$index->{$curstep}[3];
  $POSCAR=$stdir."/POSCAR";
  $stpoolsize=@$stpool;
  $stenergy=ReadStEnergyFile($stenergyfile);
  ...
  @dummy=(sort keys %$index);
  $totalsteps=@dummy;
  $totalsteps=$totalsteps-1;
  if($totalsteps > 0) {
    $previousst=$index->{$dummy[$totalsteps-1]}[2];
  }else{
    $previousst="none";
  }
...
# ($recovered,$included,$stprs,$jobs,$st)=UsePreviousGood($stdir,$stdirpre,$curstep,$numsaddles,$SearchesAlgo,$quench,$Rdiffmax,$stfile,$recovered,$included,$stprs,$index,$jobs,$st)
# ---------------------------------------------------------------------------------------------------------
sub UsePreviousGood{
  my $stdir=shift;
  my $stdirpre=shift;
  ...
  my $jobs=shift;
  my $st=shift;
  my $displacement_max=1.0;
  my ($stinfo,$POSCAR,$POSCAR_f,$POSCAR_sp,$numprst,$i,$j,$k,$same,$r_mag);
  my ($newprdir,$pr000i,$POSCAR_01,$POSCAR_02, $POSCAR_i);
  my ($coordinates1,$coordinates2,$coordinates3,$total_atoms1,$total_atoms2,$total_atoms3,$differ,$dummy);
  my ($basis,$lattice,$num_atoms,$selectiveflag,$selective,$description);
  my @rij=();
  my $suffocator="";
  if(lc($SearchesAlgo) eq "dimer") {
    ...
    ($coordinates1,$basis,$lattice,$num_atoms,$total_atoms1,$selectiveflag,$selective,$description)=read_poscar($POSCAR);
    set_bc($coordinates1,$total_atoms1);
    $POSCAR_f=$stdir."/POSCAR";
    ($coordinates2,$basis,$lattice,$num_atoms,$total_atoms2,$selectiveflag,$selective,$description)=read_poscar($POSCAR_f);
    set_bc($coordinates2,$total_atoms2);
    if($total_atoms1 != $total_atoms2) {die "$POSCAR and $POSCAR_f : different number of atoms.\n";}
    $differ=pbc_difference($coordinates2,$coordinates1,$total_atoms1);
    $differ=dirkar($differ,$basis,$lattice,$total_atoms1);
    ...
    for($i=1;$i<=$numprst;$i++) {
      ...
      if((lc($stinfo->{$pr000i}[1]) eq "good") && (lc($stinfo->{$pr000i}[0]) eq "done")) {
        ...
        ($coordinates3,$basis,$lattice,$num_atoms,$total_atoms3,$selectiveflag,$selective,$description)=read_poscar($POSCAR_sp);
        set_bc($coordinates3,$total_atoms3);
        if($total_atoms3 != $total_atoms1) {die "$POSCAR_sp and $POSCAR : different number of atoms.\n";}
        ...
        $dr=pbc_difference($coordinates3,$coordinates2,$total_atoms3);
        $dr=dirkar($dr,$basis,$lattice,$total_atoms3);
        $same=1;
        OUTMOST: for($j=0;$j<$total_atoms3;$j++)
        {
                   for($k=0;$k<3;$k++){
                     if(abs($dr->[$j][$k]) > $Rdiffmax) {
                       $same=0;
                       last OUTMOST;
                     }       
                   }       
        ...
        #$dr=pbc_difference($coordinates3,$coordinates2,$total_atoms3);
        #$dr=dirkar($dr,$basis,$lattice,$total_atoms3);
        $r_mag=magnitude($dr,$total_atoms3);
        ...
        write_poscar($coordinates3,$basis,$lattice,$num_atoms,$total_atoms3,$selectiveflag,$selective,$description,$POSCAR_01);            
...
# ---------------------------------------------------------------------------------------------------------
# Make use of good saddles at the previous step.
# ($recovered,$included,$stprs,$jobs,$st)=UseVectorShift($stdir,$stdirpre,$curstep,$numsaddles,$SearchesAlgo,$quench,$stfile,$recovered,$included,$linker,$stprs,$index,$jobs,$st)
# ---------------------------------------------------------------------------------------------------------
sub UseVectorShift{
  my $stdir=shift;
  my $stdirpre=shift;
  ...
  my $jobs=shift;
  my $st=shift;
  my ($stinfo,$POSCAR,$POSCAR_f,$POSCAR_sp,$numprst,$i,$j,$k,$r_mag);
  ...
  if(lc($SearchesAlgo) eq "dimer") {
    ...
    for($i=1;$i<=$numprst;$i++) {
      ...
      if((lc($stinfo->{$pr000i}[1]) eq "good") && (lc($stinfo->{$pr000i}[0]) eq "done")) {
        ...
        $dr=pbc_difference($coordinates3,$coordinates1,$total_atoms1);
...
# $stprdir=SurfaceVecShift($curstep,$stpool,$stenergyfile,$Ediffmax,$Rdiffmax,$index);
# ---------------------------------------------------------------------------------------------------------
sub SurfaceVecShift{
  my ($curstep,$stpool,$stenergyfile,$Ediffmax,$Rdiffmax,$index)=@_;
  my ($energy,$POSCAR,$POSCAR_j,$stpoolsize,$stenergy,@totalst,$numst,$curst,$i,$j,$similarity,$sim,$stprdir,$linker);
  my $thatisit="";
  my $finallinker="";
  $similarity=0;
  $stprdir=$index->{$curstep-1}[2]; #There is a previous st at this stage
  $energy=$index->{$curstep}[3];
  $POSCAR="POSCAR";
  $stpoolsize=@$stpool;
  $stenergy=ReadStEnergyFile($stenergyfile);
...
# $PrRecycleAll,$PrRecycle,$PrRecycleShift,$SurfaceRecShift,$stenergyfile,$stfile,$equivalency,$Ediffmax,$Rdiffmax,$DisplaceAlgo,$DisplaceRange,$NN_rcut,$MaxCoordNum,$index,$jobs,$st)
# ---------------------------------------------------------------------------------------------------------
sub PreSPSearches{
  my $stdir=shift;
  my $curstep=shift;
  ...
  my @stprs=();
  my %included=();
  my ($stdirpre,$prestep,$recovered,$reused,$sp2bcreated,$stprs,$included,$spprocessed,$i,$prdir,$prstart,$spcreated,$linker);
  $recovered=0;
  $reused=0;
  $spprocessed=0;
  $stprs=\@stprs;
  $included=\%included;
  $linker="";
  if($stdir ne $index->{$curstep}[0]) {die "Error: current st dir $stdir is not same as $index->{$curstep}[0] in the index file\n";}
  ...
  if((-e "$stdir/INCAR") && (-e "$stdir/KPOINTS") && (-e "$stdir/POTCAR") && (-e "$stdir/POSCAR") && (-e "$stdir/DISPLACECAR_sp")){
    if ($SearchesAlgo == "dimer") {
      if($PrRecycleAll) {# check good and bad saddles to see if the "final" state is same as current initial state in the current step
        ...
        ($recovered,$included,$stprs,$jobs,$st)=UseGoodBad($stdir,$curstep,$numsaddles,$SearchesAlgo,$quench,$stpool,$stenergyfile,$stfile,$recovered,$included,$equivalency,$Ediffmax,$Rdiffmax,$stprs,$index,$jobs,$st);
      ...
      if($PrRecycle) {# make a better initial guess by comparing individual atom shifts between good saddles and the current config
        ...
        sub numerically { $a <=> $b}
        @dummy=(sort numerically keys %$index);
        $prestep=@dummy;
        $prestep=$prestep-1;
        if($prestep > 0) {
          $stdirpre=$index->{$dummy[$prestep-1]}[2];
      ...
      if($PrRecycleShift) {# add each vector connecting good saddles and their own initial states to the current initial state
        ...
        if($prestep > 0) {
          if($SurfaceRecShift) { # compare configurations and try to select an equivalent or similar state
            ($stdirpre,$linker)=SurfaceVecShift($curstep,$stpool,$stenergyfile,$Ediffmax,$Rdiffmax,$index);
          }else{
            $stdirpre=$index->{$dummy[$prestep-1]}[2];
          }
          print "Previous state: $stdirpre\n";
      ...
      if($reused < $numsaddles) {
        ...
        for($i=$prstart;$i<=$spcreated;$i++){
          print "Preparing saddle point searche using random displacements\n";
          $prdir=DirName("pr",$i);
...
sub MakeSPQuench{
  ...
  if (-e "$curspdir/mins") {system "cd $curspdir; rm -r mins"};
  if (-e "$curspdir/final"){#cleanedup
    if(lc($SearchesAlgo) eq "dimer"){
...
# $curstep,$stpool,$PrRecycleAll,$PrRecycle,$PrRecycleShift,$SurfaceRecShift,$stenergyfile,$stfile,$equivalency,
# $Ediffmax,$Rdiffmax,$DisplaceAlgo,$DisplaceRange,$NN_rcut,$MaxCoordNum,%index,%jobs,%st)]
# -------------------------------------------------------------------------------------------------------
sub StartSt{
  my $curdir=shift;
  my $MaxJobs=shift; 
  ...
  ($spcreated,$jobs,$st)=PreSPSearches($curdir,$curstep,$sp2bcreated,$SearchesAlgo,$quench,$stpool,$PrRecycleAll,$PrRecycle,$PrRecycleShift,$SurfaceRecShift,$stenergyfile,$stfile,$equivalency,$Ediffmax,$Rdiffmax,$DisplaceAlgo,$DisplaceRange,$NN_rcut,$MaxCoordNum,$index,$jobs,$st);
         ...
         if($turnoff_dynmat) { # for the current run
           ...
           if(-e $curspdyndir) {
             if(-e "$curspdyndir/final") {
               ...
               $jobs->{$curspdyndir}=[("running","xulijun_tangwenjie","dynmat","na","na")];
               $st->{$prdir}[0]="dynmat";
             }
...
# $numrunjobs,$MaxJobs,$DisplaceAlgo,$DisplaceRange,$NN_rcut,$MaxCoordNum,%jobs,%st)] 
# ----------------------------------------------------------------------------------------------------
sub StartOneSP{
  my ($curdir,$prdir,$SearchesAlgo,$numrunjobs,$MaxJobs,$DisplaceAlgo,$DisplaceRange,$NN_rcut,$MaxCoordNum,$jobs,$st)=@_;
  my $curspdir=$curdir."/".$prdir;
  my $jobtype="";
  if(lc($SearchesAlgo) eq "dimer") {
...
# --------------------------------------------------------------------------------------------------
# Start a quench job [($numrunjobs,$jobs,$quenstatus,$comment)=
# StartQuench($curspdir,$SearchesAlgo,$numrunjobs,$MaxJobs,$SimR,$jobs)]
# --------------------------------------------------------------------------------------------------
sub StartQuench{
  my ($curspdir,$SearchesAlgo,$numrunjobs,$MaxJobs,$SimR,$jobs)=@_;
  my ($j,$curquendir,$quenstatus,$comment);
  my $quenjobtype="minimization";
  $quenstatus=$comment="";
  if(!(exists($jobs->{$curspdir}))) {die "In $curspdir, no job information in the jobs hash.\n"}
  MakeSPQuench($curspdir, $SearchesAlgo, $SimR); #st0001/pr0003/mins/min1/ min2/ etc.
  if(!(-e "$curspdir/mins")) {die "In $curspdir, the MakeSPQuench didn;t work successfully. CHeck it.\n";}
  for($j=1;$j<3;$j++){
  ...
  return ($numrunjobs,$jobs,$quenstatus,$comment);
}
...
# ---------------------------------------------------------------------------------------------------------------------------
sub HandleSP{
  my ($curdir,$prdir,$SearchesAlgo,$SPEnergyMax,$numrunjobs,$MaxJobs,$jobs)=@_;
  ...
  if($spstatus eq "running"){
    ...
    if($energy ne "na"  && $energy > $SPEnergyMax){ # a simpler check
      # it's going crazy or heading to a trivial saddle at best.
    ...
    if($converged){
      ...
      if($energy eq "na" || $force eq "na"){
        ...
        if($energy > $SPEnergyMax){
           #it's heading to a trivial saddle at least; don't resubmit this job
           $jobs->{$curspdir}[0]="killed";
           $comment="SaddleEnergyTooHigh";
        }else{
...
# ------------------------------------------------------------------------------------------------------------------
# Handle a quench job [($numrunjobs,$quenchst,$jobs,$comment)=HandleQuench($curspquendir,$SearchesAlgo,$numrunjobs,$MaxJobs,
...
sub HandleQuench{
  ...
  my ($quenchst,$quenjobtype,$quenjobstatus,$converged,$comment);
  ...
  return ($numrunjobs,$quenchst,$jobs,$comment);
}
...
sub FilterSP{
  ...
  my $barrier=0.0;
  my $j=0;
  my ($target, $spenergy,$status, $quality,$same, $curpsdir1, $curpsdir2, $recycled,$whereabout);
  $spenergy=$st->{$prdir}[2];
  ...
  if(($barrier >0.0) && ($barrier <= $BarrierMax)){
    ...
    for($j=1;$j<=$numprst;$j++){
      $target=DirName("pr", $j);
      if($target ne $prdir){
        #just compare it with anyone not marked as repeated, no matter whether the target saddle be good, trivial or bad,
        $status=lc($st->{$target}[0]);
        $quality=lc($st->{$target}[1]);
        if($quality eq "good" || $quality eq "bad" || $quality eq "promising" || $quality eq "trivial"){
    ...
    if($st->{$prdir}[1] eq "promising") { # found no repeat in the current state folder, how about others?
      if($GrabQuenched) {
        ($recycled,$whereabout,$st)=GrabQuenchedSP($curdir,$prdir,$SearchesAlgo,$quench,$dynmat,$stfile,$turnoff_dynmat,$equivalency,$stpool,$Ediffmax,$Rdiffmax,$DynMatFile,$stfile,$stdirs,$st);
...
# CheckSt($curdir,$numprfin,$numprrun,$numrunjobs,$MaxJobs,$NumSearches,$SearchesAlgo,$ConvergenceAlgo,
# $population,$NumKT,$Temperature,$DisplaceAlgo,$DisplaceRange,$NN_rcut,$MaxCoordNum,$jobs,$st,$st_old)]
# ---------------------------------------------------------------------------------------------------------
sub CheckSt{
  my $curdir=shift;
  my $numprfin=shift;
  ...
  my $st_old=shift;
  my $goodandsound=0;
  my ($i,$prdir,$numallowedjobs,$numprst,$numprst_new,$dummy,$dummy0, $success,$NumSearchesLeft);
  $numprst=$st->{"numprst"}[0];
  ...
  if($st->{"NumSearchesLeft"}[0] == 0) { # no need to look at processes of the state.
    ...
    SWITCH: {
      ...
      if($ConvergenceAlgo == 2) {($NumSearchesLeft,$goodandsound)=EvaluateLowEnergySPTenKT($curdir,$NumSearches,$SearchesAlgo,$NumKT,$Temperature,$population,$st_old,$st);
                            $st->{"NumSearchesLeft"}[0]=$NumSearchesLeft; #just to update st hash, not for any use in this subroutine.
                            last SWITCH;}
      if($ConvergenceAlgo == 3) {$goodandsound=EvaluateLowEnergySPPopulation($curdir,$NumSearches,$SearchesAlgo,$NumKT,$Temperature,$population,$st);
                            last SWITCH;}
    }
  }
  if($goodandsound) {
    ...
    for($i=1;$i<=$numprst;$i++) {
      ...
      if(lc($st->{$prdir}[0]) ne "done" && lc($st->{$prdir}[0]) ne "killed") {
        if (lc($st->{$prdir}[0]) eq "search") {
          if($jobs->{$dummy}[0] eq "running" || $jobs->{$dummy}[0] eq "queue" || $jobs->{$dummy}[0] eq "submitted") {
            $success=KillJobs($dummy, $jobs->{$dummy}[1]);
            #print "tried killing a job in $dummy jobid:".$jobs->{$dummy}[1]."\n";
            #if(!$success) {print "can not kill job ".$jobs->{$dummy}[1]. "in $dummy.\n";}
          }
          if(-e "$dummy/OUTCAR") { CleanupJobs($dummy,0,$SearchesAlgo); }
        }elsif(lc($st->{$prdir}[0]) eq "quench") {
          $dummy0=$dummy;
          $dummy=$dummy0."/mins/min1";
          if($jobs->{$dummy}[0] eq "running" || $jobs->{$dummy}[0] eq "queue" || $jobs->{$dummy}[0] eq "submitted") {
            $success=KillJobs($dummy, $jobs->{$dummy}[1]);
            #print "tried killing a job in $dummy jobid:".$jobs->{$dummy}[1]."\n";
            #if(!$success) {print "can not kill job ".$jobs->{$dummy}[1]. "in $dummy.\n";}
          }    
          if(-e "$dummy/OUTCAR") {CleanupJobs($dummy, 0, "minimization");}
          $dummy=$dummy0."/mins/min2";
          if($jobs->{$dummy}[0] eq "running" || $jobs->{$dummy}[0] eq "queue" || $jobs->{$dummy}[0] eq "submitted") {
            $success=KillJobs($dummy, $jobs->{$dummy}[1]);
            #print "tried killing a job in $dummy jobid:".$jobs->{$dummy}[1]."\n";
            #if(!$success) {print "can not kill job ".$jobs->{$dummy}[1]. "in $dummy.\n";}
          ...
          if(-e "$dummy/OUTCAR") {CleanupJobs($dummy, 0, "dynmat");}
        }else{
          #print "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n";
    ...
    if($numallowedjobs > 0) {
      ...
      for($i=$numprst+1;$i<=$numprst_new;$i++) {
        $prdir=DirName("pr", $i);
        ($numrunjobs,$jobs,$st)=StartOneSP($curdir,$prdir,$SearchesAlgo,$numrunjobs,$MaxJobs,$DisplaceAlgo,$DisplaceRange,$NN_rcut,$MaxCoordNum,$jobs,$st);
        $numprrun++;
      }
      $st->{"status"}[0]="running";
...
#  $time_saved,$Rdiffmax,$RateTableFile,$Temperature,$RateTableFileBolEqu,$index,$curstep,$BoltzmanEqu)
# ----------------------------------------------------------------------
sub SelectEvent{
  my ($rt,$curdir,$prest,$rdnum,$rdnum2,$enforced,$t_saved,$Rdiffmax,$RTFile,$Temp,$RTFileBE,$index,$curstep,$BEqu)=@_;
  ...
  if($enforced eq ""){
    ...
    if(!$rtbe){
      ...
      if($dominant ne ""){ 
       ($BolEqu,$rtbe)=CheckBoltzman($rt,$curdir,$prest,$BEqu,$dominant,$Rdiffmax,$RTFile,$Temp,$RTFileBE,$index,$curstep);
      }
    }else{
      $BolEqu=1;
    }
...
# ---------------------------------------------------------------------------------------------------------
# ($BolEqu,$rt)=CheckBoltzman($rt,$curdir,$prest,$BoltzmanEqu,$dominant,$Rdiffmax,$RateTableFile,$Temperature,$RateTableFileBolEqu,$index,$curstep)
# ---------------------------------------------------------------------------------------------------------
sub CheckBoltzman{
  my ($rt,$curdir,$prest,$BoltzmanEqu,$dominant,$Rdiffmax,$RateTableFile,$Temperature,$RateTableFileBolEqu,$index,$curstep)=@_;
  ...
  if($same){ # check if the prest also has a dominant process
    ...
    if($dominant2 ne ""){ 
      ...
      if($same){ # bingo
        $deltaE=$index->{$curstep}[3]-$index->{$curstep-1}[3];
        $line="";
        for $i (sort keys %$rt){
        ...
        for $i (sort keys %$rtpre){
          ...
          if($deltaE>0){ # prest is of the lower energy
            ...
            $line.=$curdir."/$i ".$dummy." @{$rtpre->{$i}}[1 .. $j] ";
...
# ---------------------------------------------------------------------------------------------------------
# WriteIndex($AkmcFile,$index,$initial)
# ---------------------------------------------------------------------------------------------------------
sub WriteIndex{
  my ($AkmcFile,$index,$initial)=@_;
  my $AkmcFile=shift;
  my $index=shift;
  my $initial=shift;
  my $j="";
  ...
  if($initial) {
    ...
    print OUT "akmcStep stnumber property target stenergy(eV) ftenergy(eV) totaltime(s)"."\n";
  }else{
    open(OUT, ">>$AkmcFile") || die "Error in opening $AkmcFile file for writing KMC step information.\n";
  }
  sub numerically { $a <=> $b}
  @newline= (sort numerically keys %$index);
  $j=@newline;
  if(!$j) {die "Error in WriteIndex: the index has no elements.\n";}
  print OUT $newline[$j-1]." "."@{$index->{$newline[$j-1]}}"."\n";
  close OUT;
}

# ----------------------------------------------------------------------------------------------------------------------------
# Update state by making a move
# ($curstep,$st,$index)=UpdateSt($curstep,$repeat,$selected,$randomnum,$time,$StFile,$StEnergyFile,$AkmcFile,$st,$index);
# ----------------------------------------------------------------------------------------------------------------------------
sub UpdateSt{
  my $curstep=shift;
  my $repeat=shift;
  ...
  $curdir=$index->{$curstep}[2];
  $stname=$index->{$curstep}[0];
  if($stname ne substr($curdir, -6)) {die "$index for $curstep is messed up.\n";}
  ...
  $index->{$curstep}[4]=$energy;
  # accumulate the total time
  $index->{$curstep}[5]=$index->{$curstep}[5] + $time_elapse;
  $total_time=$index->{$curstep}[5];
  print "Time for this step is: $time_elapse\n";
  print "Total time elapsed is: $total_time\n";
  # save the done step information
  if($curstep == 1) {
    WriteIndex($AkmcFile, $index, 1);
  }else {
    WriteIndex($AkmcFile, $index, 0);
  }
  # put a new line to the index file and keeps index from having more than two elements
  $prestep=$curstep-1;
  if(exists($index->{$prestep})) {delete $index->{$prestep};}
  if(exists($index->{""})) {delete $index->{""};}
  # $curstep=1+(keys %$index);
  $curstep=$curstep+1;
  # $curstep=DirName("Step",$curstep);
  $index->{$curstep}=[()];
  push @{$index->{$curstep}}, "2bcreated", "unknown", "unknown",$energy,"na",$total_time;
  return ($curstep,$st,$index);
}
