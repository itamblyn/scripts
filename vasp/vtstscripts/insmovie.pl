...
# Go over all the subdirectories and create the first timeframe.
  ...
  while (-e $dir) {
    chdir $dir ;
    print " --> ",$dir,"\n" ;
    system "vasp2con.pl POSCAR >& $blackhole ; con2xyz.pl POSCAR.con >& $blackhole" ;
    chdir $home ;
    $i++ ;
    if ($i < 10) {$dir = "0$i" ;}
    elsif ($i < 100) {$dir = "$i" ;}
...
# Then extract the step index where the system is moved and generate a timeframe.
  ...
  for ($j = 1 ; $j < $nt ; $j++) {
    ...
    while (-e $dir) {
      chdir $dir ;
      print " --> ",$dir,"\n" ;
      system "xdat2pos.pl $fc+1" ; 
      rename "POSCAR.out" , "ptmp" ; 
      system "vasp2con.pl ptmp >& $blackhole" ; 
      system "con2xyz.pl ptmp.con >& $blackhole" ; 
      chdir $home ;
      $i++ ;
      if ($i < 10) {$dir = "0$i" ;}
      elsif ($i < 100) {$dir = "$i" ;}
