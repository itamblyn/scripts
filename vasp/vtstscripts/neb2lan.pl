...
# Select the image from the exts.dat file
  ...
  $d1 = sprintf "%02d",$i ;
  $d2 = sprintf "%02d",$i+1 ;
  open (DIST, "dist.pl $d1/POSCAR $d2/POSCAR |") ;
  $d12 = <DIST> ; close DIST ;
...
# Generate the dimer POSCARs.
  ...
  system "interpolate.pl $d1/POSCAR $d2/POSCAR $f1 > /dev/null" ;
  rename "POSCAR.out" , "POSCAR1" ;
  system "interpolate.pl $d1/POSCAR $d2/POSCAR $f2 > /dev/null" ;
  rename "POSCAR.out" , "POSCAR2" ;
  system "interpolate.pl $d1/POSCAR $d2/POSCAR $f > /dev/null" ;
  ...
  if(-e "POTCAR"){
    ...
    system "cp ../POTCAR $dir" ; }
  system "modemake.pl POSCAR1 POSCAR2 ; mv MODECAR $dir ; mv POSCAR $dir" ;
