...
# Last Modified Jan. 29, 2007 by Lijun Xu, at Austin, TX
#----------------------------------------------------------------------
...
(-e "$poscarfilename") || die (" POSCAR file $poscarfilename does not exist\n");
(-e "$modecar") || die (" POSCAR file $modecar does not exist\n");
...
if(!$replace_this_poscar) {
  ...
  write_poscar($image1,$basis,$lattice,$num_atoms,$total_atoms,$selectiveflag,$selective,$description,$tmpfilename);
  system "mv $tmpfilename $minsfolder/min1/POSCAR";
  write_poscar($image2,$basis,$lattice,$num_atoms,$total_atoms,$selectiveflag,$selective,$description,$tmpfilename);
