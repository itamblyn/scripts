...
##  Loop over frames in the molanal.out file.	
    open (H3O, ">h3o.dat")  || die "Can't open h3o.dat\n";
    open (OHM, ">oh.dat")   || die "Can't open ohm.dat\n";
    open (HP,  ">hp.dat")   || die "Can't open hp.dat\n";
    open (H2O, ">h2o.dat")  || die "Can't open h2o.dat\n";
    open (OM,  ">om.dat")   || die "Can't open om.dat\n";
    open (OO,  ">O-O.dat")  || die "Can't open O-O.dat\n";
    open (OH,  ">O-H.dat")  || die "Can't open O-H.dat\n";
    open (ON,  ">O-N.dat")  || die "Can't open O-N.dat\n";
    open (OC,  ">O-C.dat")  || die "Can't open O-C.dat\n";
    open (HH,  ">H-H.dat")  || die "Can't open H-H.dat\n";
    open (HN,  ">H-N.dat")  || die "Can't open H-N.dat\n";
    open (HC,  ">H-C.dat")  || die "Can't open H-C.dat\n";
    open (NN,  ">N-N.dat")  || die "Can't open N-N.dat\n";
    open (CN,  ">C-N.dat")  || die "Can't open C-N.dat\n";
    open (CC,  ">C-C.dat")  || die "Can't open C-C.dat\n";
    do {
...
##      See which molecules are new, and which have been found before.
	...
	@reactants = () ;
		...
		$molcount{$mol_name{$i}} += 1 ;
                $mol_count++;
                $mol_tot++;
                if ($mol_name{$i} =~ /O1\ H3\ 3\(H-O\)/ ) {
                  $h3o_count++;
                } elsif ($mol_name{$i} =~ /O1\ H1\ 1\(H-O\)/ ) {
                  $ohm_count++;
                } elsif ($mol_name{$i} =~ 'H1' ) {
                  $h_count++;
                } elsif ($mol_name{$i} =~ /O1\ H2\ 2\(H-O\)/ ) {
                  $h2o_count++;
                } elsif ($mol_name{$i} =~ 'O1' ) {
                  $o_count++;
                }
...
##              Accumulate averages of quantities of interest.
        ...
        $h3o_count /= $mol_count;
        print (H3O "$frame_count $h3o_count\n");
        $ohm_count /= $mol_count;
        print (OHM "$frame_count $ohm_count\n");
        $h_count /= $mol_count;
        print (HP "$frame_count $h_count\n");
        $o_count /= $mol_count;
        print (OM "$frame_count $o_count\n");
        $h2o_count /= $mol_count;
        print (H2O "$frame_count $h2o_count\n");
#
        $oo_count /= $volume[$frame_count];
        print (OO "$frame_count $oo_count\n");
#
        $oh_count /= $volume[$frame_count];
        print (OH "$frame_count $oh_count\n");
#
        $on_count /= $volume[$frame_count];
        print (ON "$frame_count $on_count\n");
#
        $oc_count /= $volume[$frame_count];
        print (OC "$frame_count $oc_count\n");
#
        $hh_count /= $volume[$frame_count];
        print (HH "$frame_count $hh_count\n");
#
        $hn_count /= $volume[$frame_count];
        print (HN "$frame_count $hn_count\n");
#
        $hc_count /= $volume[$frame_count];
        print (HC "$frame_count $hc_count\n");
#
        $nn_count /= $volume[$frame_count];
        print (NN "$frame_count $nn_count\n");
#
        $cn_count /= $volume[$frame_count];
        print (CN "$frame_count $cn_count\n");
#
        $cc_count /= $volume[$frame_count];
        print (CC "$frame_count $cc_count\n");
#
