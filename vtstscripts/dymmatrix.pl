...
#  $nions_typ = `grep 'ions per type =' OUTCAR` ;
  ...
  $line = $nions_typ ; $line=~s/^\s+//g ; @line=split /\s+/,$line ;
  @natoms_typ = @line[4..4+$ntyp] ;
  for($i=0 ; $i<$ntyp ; $i++) {
    $line = $pomass[$i] ; $line=~s/^\s+//g ; @line=split /\s+/,$line ;
    @m = split /;/ , $line[2] ;
    $typ_mass[$i] = $m[0] ;
  ...
  for ($i=0;$i<$total_atoms;$i++) {
	...
	for ($n=0;$n<$num_displacecars;$n++) {
	  for ($k=0;$k<$total_atoms;$k++) {
		...
		$matrix->[$current_displacement][$this_displacement]=$forces->[$current_displacement][$k][$l]
		  /$displacecar->[$m][$i][$j];
# AA ... mass-scaling
                ...
                $mass_k=$mass[$k];
                $mass_i=$mass[$i];
                print "$current_displacement $this_displacement $mass_k $mass_i";
# AA
		$this_displacement++;
	      }
	    }
...
# Sort the eigenvalues. Use selection sort. $t will be the order we want to print out
  ...
  for($i=0 ; $i<$num_dof ; $i++) {
    $iptr = $i ;
    for($j=$i+1 ; $j<$num_dof ; $j++) {if($w[$j] < $w[$iptr]){$iptr = $j ;}}
    if($i != $iptr) {
      $tmp = $w[$i] ;
      $w[$i] = $w[$iptr] ;
      $w[$iptr] = $tmp ;
