...
#
  ...
  $line = @cf1[2] ; chomp($line) ; $line=~s/^\s+//g ; @line=split /\s+/,$line ;
  @box = @line[0..2] ;
  $line = @cf1[6] ; chomp($line) ; $line=~s/^\s+//g ; @line=split /\s+/,$line ;
  $ntyp = @line[0] ;
  $line = @cf1[7] ; chomp($line) ; $line=~s/^\s+//g ; @line=split /\s+/,$line ;
